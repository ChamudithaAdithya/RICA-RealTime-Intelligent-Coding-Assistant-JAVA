...
exports.crossLayerViolationRule = exports.entityExposureRule = exports.cyclicDependencyRule = exports.controllerBypassRule = exports.ProjectDependencyGraph = void 0;
...
exports.patchGraphForFile = patchGraphForFile;
class ProjectDependencyGraph {
    ...
    ensureNode(id, type, metadata) {
        if (!this.nodes.has(id)) {
            this.nodes.set(id, { id, type, metadata });
        }
    }
    addEdge(source, target, type, location) {
    ...
    resolveTypeFQN(simpleName, fileImports, ownPackage) {
     ...
     */
    reachable(source, target, maxHops) {
        if (source === target)
            return true;
        const adj = this.buildClassAdjacency();
        ...
        while (queue.length > 0) {
            ...
            if (maxHops !== undefined && depth >= maxHops)
                continue;
...
function addFileToGraph(graph, filePath, ast) {
    const pkgName = ast.packageInfo?.name || '';
    graph.addNode(filePath, 'file', {
        filePath,
        packageName: pkgName,
...
function removeFileFromGraph(graph, filePath) {
    ...
    graph.edges = graph.edges.filter(e => !nodeIdsToRemove.includes(e.source) && !nodeIdsToRemove.includes(e.target));
    // Remove the nodes
    for (const id of nodeIdsToRemove) {
        graph.nodes.delete(id);
    }
}
function patchGraphForFile(graph, filePath, oldAst, newAst, allFiles) {
...
function resolveEdgeTarget(targetId, graph, imports, ownPackage) {
...
exports.cyclicDependencyRule = {
    id: 'CYCLIC_DEP',
    name: 'Cyclic or inverted dependency',
    severity: 'error',
    analyze(graph, _files) {
        const violations = [];
...
exports.crossLayerViolationRule = {
    id: 'CROSS_LAYER',
