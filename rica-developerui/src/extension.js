...
async function exportAnalysisSnapshot() {
    const workspaceFolders = vscode.workspace.workspaceFolders;
    if (!workspaceFolders || workspaceFolders.length === 0) {
    ...
    if (Object.keys(violationManager.getFilesMap()).length === 0) {
        const answer = await vscode.window.showInformationMessage('RICA has no AST cache yet. Analyze the full project first?', 'Analyze Now', 'Export Empty Snapshot', 'Cancel');
    ...
    const snapshot = violationManager.getAnalysisSnapshot();
    ...
    await vscode.workspace.fs.writeFile(vscode.Uri.joinPath(snapshotRoot, 'README.md'), Buffer.from(readme, 'utf8'));
...
function safeSnapshotFileName(filePath) {
    return filePath
        .replace(/^[A-Za-z]:/, '')
        .replace(/[\\/]+/g, '__')
