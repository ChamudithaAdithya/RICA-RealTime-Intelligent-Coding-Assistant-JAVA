...
export class JavaParser {
    ...
    private cstToAst(cst: any, filePath: string, sourceCode: string): FullASTOutput {
        if (!cst) {
            return {
                    ...
                    for (const call of method.calledMethods) {
                        if (call.targetClass && !call.isLibraryCall) {
                            ...
                            const existing = relationships.find(r =>
                                r.sourceId === cls.fullyQualifiedName &&
                            ...
                            if (!existing) {
                                relationships.push(this.makeRelation(
                                    cls.fullyQualifiedName,
                                    call.targetClass,
                                    'calls',
                                    `calls ${call.calledMethodName}`,
                                    {
    ...
    private createEmptyPackageInfo(filePath: string): PackageInfo {
        return {
            name: 'default',
            simpleName: 'default',
            parentPackage: null,
    ...
    private extractPackageInfo(cst: any, filePath: string): PackageInfo {
    ...
    private findPackageDeclaration(node: any): string | null {
        if (!node) return null;
    ...
    private getIdentifierFromNode(node: any): string {
        if (!node?.children) return '';

        if (node.children.Identifier) {
            const identifiers: string[] = [];
            for (const id of node.children.Identifier) {
                if (id.image) identifiers.push(id.image);
            }
    ...
    private getClassModifiersFromNode(node: any): string[] {
        const modifiers: string[] = [];
        if (!node?.children?.classModifier) return modifiers;
        for (const cm of node.children.classModifier) {
            if (cm.children) {
                if (cm.children.Public) modifiers.push('public');
                if (cm.children.Private) modifiers.push('private');
                if (cm.children.Protected) modifiers.push('protected');
                if (cm.children.Abstract) modifiers.push('abstract');
                if (cm.children.Static) modifiers.push('static');
                if (cm.children.Final) modifiers.push('final');
                if (cm.children.Strictfp) modifiers.push('strictfp');
                if (cm.children.Sealed) modifiers.push('sealed');
                if (cm.children.NonSealed) modifiers.push('non-sealed');
            }
        }
        return modifiers;
    }

    private getAnnotationsFromModifierList(modifiers: any[]): Annotation[] {
        const annotations: Annotation[] = [];
        for (const mod of modifiers) {
            if (mod.children?.annotation) {
                for (const ann of mod.children.annotation) {
                    const annObj = this.extractAnnotation(ann);
                    if (annObj) annotations.push(annObj);
                }
            }
        }
        return annotations;
    ...
    private processNormalClass(
        ncd: any,
        classModifiers: string[],
        classModifierNodes: any[],
        ...
        if (superClass) {
            relationships.push(this.makeRelation(
                fullyQualifiedName,
                superClass === 'Object' ? 'java.lang.Object' : superClass,
                'extends',
                'inherits',
                { cardinality: 1 }
            ));
        ...
        const { fields, methods, constructors, staticInitBlocks, instanceInitBlocks, innerClassNames } =
            this.processClassBody(ncd, className, sourceCode, allAnnotations);

        // Determine class type
        let classType: ClassInfo['classType'] = 'class';
        if (isAbstract) classType = 'abstract class';
        if (isSealed) classType = 'sealed class';
        // Note: 'final' is indicated by isFinal flag, not by classType

        // Determine access modifier
        const accessModifier = this.getAccessModifier(modifiers) as ClassInfo['accessModifier'];
        ...
        const classInfo: ClassInfo = {
            ...
            ...loc,
            accessModifier,
            nonAccessModifiers: modifiers.filter(m =>
                !['public', 'protected', 'private', 'package-private'].includes(m)
            ),
            classType,
        ...
        if (allInjected.length > 0) {
            ...
            const hasConstructorAssign = allInjected.some(a =>
                a.injectionType === 'implicit-constructor' || a.injectionType === 'lombok-constructor'
            );
            ...
            if (uniqueTypes.size > 1) classInfo.injectionStrategy = 'mixed';
            else if (hasFieldAnnotation) classInfo.injectionStrategy = 'field';
            else if (hasConstructorAssign) classInfo.injectionStrategy = 'constructor';
            else if (hasSetterInjection) classInfo.injectionStrategy = 'setter';
            else classInfo.injectionStrategy = 'mixed';
        } else {
        ...
        const hasARelMap = new Map<string, { count: number, isInjected: boolean, injectionTypes: Set<string> }>();
        ...
        for (const [key, entry] of hasARelMap) {
            ...
            relationships.push(this.makeRelation(
                ...
                to,
                'has-a',
                entry.count > 1 ? `owns (${entry.count}x)` : 'owns',
                {
    ...
    private makeRelation(from: string, to: string, type: Relationship['type'], label: string, metadata?: RelationshipMetadata): Relationship {
        return { sourceId: from, targetId: to, type, label, metadata };
    }

    private extractPermittedSubclasses(node: any): string[] {
        if (!node?.children?.permits) return [];
        const permits = node.children.permits[0];
        if (!permits?.children?.classTypeList) return [];
        const ctl = permits.children.classTypeList[0];
        if (!ctl?.children?.classType) return [];

        return ctl.children.classType.map((ct: any) => this.extractClassName(ct)).filter(Boolean);
    }
    ...
    private processEnumDeclaration(node: any, classes: ClassInfo[], sourceCode: string, filePath: string): void {
        ...
        const accessModifier = this.getAccessModifier(modifiers) as ClassInfo['accessModifier'];
        const nonAccessModifiers = modifiers.filter(m =>
            !['public', 'protected', 'private', 'package-private'].includes(m)
        );
        ...
        const attributes: Attribute[] = constants.map(c => ({
            name: c,
            dataType: enumName,
        ...
        const methods: Method[] = []; // Could extract from enum body
    ...
    private extractEnumDetails(node: any, enumName: string, fullyQualifiedName: string, accessModifier: 'public' | 'package-private', sourceCode: string): Enum {
        const implementsInterfaces = this.getExtendsInterfaces(node);
    ...
    private extractEnumConstants(node: any): any[] {
        const constants: any[] = [];

        if (!node?.children?.enumBody) return constants;

        const enumBody = node.children.enumBody[0];
        if (!enumBody?.children?.enumConstantList) return constants;

        const enumConstantList = enumBody.children.enumConstantList[0];
        if (!enumConstantList?.children?.enumConstant) return constants;
    ...
    private extractEnumConstantArgs(ec: any): any[] {
        const args: any[] = [];

        if (!ec?.children?.arguments) return args;

        const argumentsNode = ec.children.arguments[0];
        if (!argumentsNode?.children?.expressionList) return args;

        const el = argumentsNode.children.expressionList[0];
        if (!el?.children?.expression) return args;
    ...
    private extractExpressionValue(expr: any): any {
        // Simple extraction for literals
        if (!expr?.children) return null;
    ...
    private extractEnumFields(node: any): { name: string; dataType: string }[] {
        const fields: { name: string; dataType: string }[] = [];

        // Enum can have fields in its body
        const enumBody = node.children?.enumBody?.[0];
        if (!enumBody?.children?.classBodyDeclaration) return fields;
    ...
    private extractEnumConstructor(enumName: string): string {
     ...
     */
    private extractEnumMethods(node: any): string[] {
        ...
        const enumBody = node?.children?.enumBody?.[0];
        if (!enumBody?.children?.classBodyDeclaration) return methods;

        const classBodyDecls: any[] = enumBody.children.classBodyDeclaration || [];
        for (const cbd of classBodyDecls) {
            const classMemberDecls: any[] = cbd?.children?.classMemberDeclaration || [];
    ...
    private extractMethodNameFromDeclaration(md: any): string {
    ...
    private processNormalInterface(
        nid: any,
        classes: ClassInfo[],
        relationships: Relationship[],
        sourceCode: string
        ...
        const { abstractMethods, defaultMethods, staticMethods, privateMethods, constants } =
            this.processInterfaceBody(nid);
     ...
     */
    private buildFabricatedMethod(name: string, methodType: 'abstract' | 'default' | 'static'): Method {
        return {
            name,
            accessModifier: 'public',
    ...
    private processAnnotationInterface(node: any, classes: ClassInfo[], sourceCode: string, filePath: string): void {
        ...
        const loc = this.extractLocation(node);
        const annotationInfo: ClassInfo = {
    ...
    private getTypeIdentifier(node: any): string {
        if (!node?.children) return '';
    ...
    private getSuperInterfaces(node: any): string[] {
    ...
    private getExtendsInterfaces(node: any): string[] {
    ...
    private extractTypeList(node: any): string[] {
        const types: string[] = [];
    ...
    private getEnumConstants(node: any): string[] {
    ...
    private processClassBody(
        ...
        staticInitBlocks: { block: string }[];
        instanceInitBlocks: { block: string }[];
        innerClassNames: string[];
    } {
        const fields: Attribute[] = [];
        const methods: Method[] = [];
        const constructors: Constructor[] = [];
        const staticInitBlocks: { block: string }[] = [];
        const instanceInitBlocks: { block: string }[] = [];
        ...
        for (const cbd of classBody.children.classBodyDeclaration) {
            ...
            if (cbd.name === 'staticInitializer') {
                if (cbd.children?.classInitializer) {
                    ...
                    if (block) staticInitBlocks.push(block);
                }
                continue;
            }
            ...
            if (cbd.name === 'initializer') {
                const block = this.extractInitializerBlock(cbd);
                if (block) instanceInitBlocks.push(block);
                continue;
            }
            ...
            if (cbd.children?.constructorDeclaration) {
                ...
                if (ctor) constructors.push(ctor);
            }
        ...
        const hasLombokAnnotation = classAnnotations.some(a =>
            lombokAnnotations.some(l => a.name === l || a.fullyQualifiedName === l)
        );
        if (hasLombokAnnotation) {
            const isRequired = classAnnotations.some(a =>
                a.name === 'RequiredArgsConstructor' || a.fullyQualifiedName === 'RequiredArgsConstructor'
            );
            if (isRequired) {
    ...
    private extractFieldAnnotations(fd: any): Annotation[] {
        const annotations: Annotation[] = [];
    ...
    private isInjectionAnnotation(annotations: Annotation[]): boolean {
    ...
    private processInterfaceBody(node: any): {
        abstractMethods: string[];
        defaultMethods: { name: string; body: string }[];
        staticMethods: { name: string }[];
        privateMethods: string[];
        constants: { name: string; dataType: string; value: any }[];
        /** Signature metadata for every interface method so downstream signature
         *  rules (fat-interface usage ratio, call-site matching) see real types. */
        methodSignatures: { name: string; returnType: string; parameterTypes: string[] }[];
    } {
        const abstractMethods: string[] = [];
        const defaultMethods: { name: string; body: string }[] = [];
        const staticMethods: { name: string }[] = [];
        const privateMethods: string[] = [];
        const constants: { name: string; dataType: string; value: any }[] = [];
        const methodSignatures: { name: string; returnType: string; parameterTypes: string[] }[] = [];
    ...
    private getInterfaceMethodParameterTypes(iMethod: any): string[] {
        const declarator = iMethod?.children?.methodHeader?.[0]?.children?.methodDeclarator?.[0];
        if (!declarator?.children?.formalParameterList) return [];
        const fpl = declarator.children.formalParameterList[0];
        if (!fpl?.children?.formalParameter) return [];
        const types: string[] = [];
    ...
    private getFieldModifiers(node: any): string[] {
        const modifiers: string[] = [];
        if (!node?.children?.fieldModifier) return modifiers;

        for (const fm of node.children.fieldModifier) {
            if (fm.children) {
                if (fm.children.Public) modifiers.push('public');
                if (fm.children.Private) modifiers.push('private');
                if (fm.children.Protected) modifiers.push('protected');
                if (fm.children.Static) modifiers.push('static');
                if (fm.children.Final) modifiers.push('final');
            }
        }
    ...
    private getMethodModifiers(node: any): string[] {
        const modifiers: string[] = [];
        const modKey = node?.children?.methodModifier
            ? 'methodModifier'
            : node?.children?.interfaceMethodModifier ? 'interfaceMethodModifier' : null;
        if (!modKey) return modifiers;

        for (const mm of node.children[modKey]) {
            if (mm.children) {
                if (mm.children.Public) modifiers.push('public');
                if (mm.children.Private) modifiers.push('private');
                if (mm.children.Protected) modifiers.push('protected');
                if (mm.children.Abstract) modifiers.push('abstract');
                if (mm.children.Static) modifiers.push('static');
                if (mm.children.Final) modifiers.push('final');
            }
        }
    ...
    private getConstructorModifiers(node: any): string[] {
        const modifiers: string[] = [];
        if (!node?.children?.constructorModifier) return modifiers;
    ...
    private getFieldType(node: any): string {
        if (!node?.children?.unannType) return 'unknown';
        ...
        if (node.children?.unannPrimitiveType) {
            ...
            if (pt.children?.numericType) {
                ...
                if (nt.children?.integralType) {
                    ...
                    if (it.children?.Int) return 'int';
                    if (it.children?.Long) return 'long';
                    if (it.children?.Short) return 'short';
                    if (it.children?.Byte) return 'byte';
                    if (it.children?.Char) return 'char';
                }
                if (nt.children?.floatingPointType) {
                    const ft = nt.children.floatingPointType[0];
                    if (ft.children?.Float) return 'float';
                    if (ft.children?.Double) return 'double';
                }
            }
            if (pt.children?.Boolean) return 'boolean';
        }
    ...
    private serializeTypeNode(node: any): string {
        const tokens: any[] = [];
    ...
    private collectTerminalTokens(node: any, tokens: any[]): void {
        if (!node) return;
    ...
    private extractClassName(node: any): string {
        if (!node?.children?.unannClassType) return '';
    ...
    private extractTypeArguments(node: any): string | null {
        if (!node?.children?.unannClassType) return null;
        
        const uct = node.children.unannClassType[0];
        if (!uct.children?.typeArguments) return null;
        
        const ta = uct.children.typeArguments[0];
        if (!ta.children?.typeArgumentList) return null;
        
        const tal = ta.children.typeArgumentList[0];
        if (!tal.children?.typeArgument) return null;
        
        const typeArg = tal.children.typeArgument[0];
        if (!typeArg.children?.referenceType) return null;
        
        const rt = typeArg.children.referenceType[0];
        if (!rt.children?.classOrInterfaceType) return null;
        
        const cit = rt.children.classOrInterfaceType[0];
        if (!cit.children?.classType) return null;
        
        const ct = cit.children.classType[0];
        if (!ct.children?.Identifier) return null;
        
        return ct.children.Identifier[0].image;
    }

    private getVariableDeclaratorIds(node: any): string[] {
        const names: string[] = [];
        
        if (!node?.children?.variableDeclaratorList) return names;
        
        const vdl = node.children.variableDeclaratorList[0];
        if (!vdl?.children?.variableDeclarator) return names;
    ...
    private extractDetailedMethod(
        ...
        if (!node?.children?.methodHeader) return null;
        ...
        const isAbstract = modifiers.includes('abstract');
        const accessModifier = this.getAccessModifier(modifiers) as Method['accessModifier'];
        const nonAccessModifiers = modifiers.filter(m =>
            !['public', 'protected', 'private', 'package-private'].includes(m)
        );
        ...
        let methodType: Method['methodType'] = 'instance';
        if (nonAccessModifiers.includes('static')) methodType = 'static';
        if (nonAccessModifiers.includes('synchronized')) methodType = 'synchronized';
        if (isAbstract) methodType = 'abstract';
        if (nonAccessModifiers.includes('native')) methodType = 'native';

        // Check for default method (interface)
        if (nonAccessModifiers.includes('default')) methodType = 'default';
        ...
        const symbolTable = new Map<string, { type: string, isInjected: boolean }>();
    ...
    private extractMethodCalls(methodBody: any, symbolTable?: Map<string, { type: string, isInjected: boolean }>): MethodCall[] {
        const calls: MethodCall[] = [];
        if (!methodBody) return calls;



        const extractArgumentsFromSuffix = (suffix: any): string[] => {
            const args: string[] = [];
            const mis = suffix.children?.methodInvocationSuffix?.[0];
            if (!mis?.children?.argumentList) return args;
            const al = mis.children.argumentList[0];
            if (!al?.children?.expression) return args;
        ...
        const collectFqnIdentifiers = (fqnNode: any): string[] => {
            const ids: string[] = [];
            const collect = (n: any) => {
                if (!n) return;
                if (n.name === 'fqnOrRefTypePartCommon' && n.children?.Identifier) {
                    for (const id of n.children.Identifier) {
                        if (id.image) ids.push(id.image);
                    }
                }
                if (n.children) {
                    for (const key of Object.keys(n.children)) {
        ...
        const traverse = (node: any) => {
            ...
            if (node.name === 'primarySuffix' && node.children?.methodInvocationSuffix) {
                ...
                const findPrimary = (n: any, parent: any): boolean => {
                    if (n === node) { primaryNode = parent; return true; }
                    if (n.children) {
                        for (const key of Object.keys(n.children)) {
                            if (Array.isArray(n.children[key])) {
                                for (const c of n.children[key]) {
                                    if (findPrimary(c, n)) return true;
                                }
                            }
                        }
                    }
            ...
            if (node.name === 'primary') {
                ...
                if (prefix && suffix && suffix.children?.methodInvocationSuffix) {
                    ...
                    if (fqnIdentifiers.length > 0) {
                        ...
                        if (loc?.startLine) lineNumber = loc.startLine;
            ...
            if (node.name === 'methodCall' && node.children?.methodName) {
                ...
                if (methodName) {
                    ...
                    let targetClass: string | undefined;
                    let receiverVarName: string | undefined;
                    let receiverType: string | undefined;
                    let receiverIsInjected = false;
    ...
    private getReceiverVariableName(receiver: any): string | undefined {
    ...
    private extractCreationArgs(node: any): string[] {
        const args: string[] = [];
        const collectArgs = (n: any): void => {
            if (!n) return;
    ...
    private containsConditionalExpression(node: any): boolean {
        ...
        if (node.name === 'conditionalExpression' && node.children?.QuestionMark) return true;
        if (node.children) {
            for (const key of Object.keys(node.children)) {
                if (Array.isArray(node.children[key])) {
                    for (const child of node.children[key]) {
                        if (child !== node && this.containsConditionalExpression(child)) return true;
                    }
                }
            }
        }
    ...
    private readonly PERSISTENCE_TYPE_RE = /(Repository|Dao|DAO|EntityManager|JdbcTemplate|JdbcOperations|Session|MongoTemplate|Mapper|PersistenceManager)$/;
    private readonly PERSISTENCE_READ_RE = /^(find|get|read|count|exists|has|load|query|search|is|to)/i;

    private extractPersistenceWrites(calledMethods: MethodCall[]): PersistenceWrite[] {
        const writes: PersistenceWrite[] = [];
        for (const call of calledMethods) {
            const receiverType = call.receiverType || call.targetClass || '';
            if (!this.PERSISTENCE_TYPE_RE.test(receiverType.replace(/<.*>/g, '').trim())) continue;
            if (this.PERSISTENCE_READ_RE.test(call.calledMethodName)) continue;
            writes.push({
    ...
    private extractWrittenVariables(methodBody: any): string[] {
        ...
        const traverse = (node: any) => {
            ...
            if (node.name === 'localVariableDeclaration') {
                ...
                for (const n of names) written.add(n);
            }
            ...
            if (node.children) {
                for (const key of Object.keys(node.children)) {
                    if (Array.isArray(node.children[key])) {
                        for (const child of node.children[key]) traverse(child);
                    }
                }
            }
        };
    ...
    private collectLhsIdentifiers(node: any): string[] {
        if (!node) return [];
        const ids: string[] = [];
        const collect = (n: any) => {
            if (!n) return;
    ...
    private extractDetailedMethodParameters(header: any, fieldNames: Set<string>): Parameter[] {
        const params: Parameter[] = [];

        if (!header?.children?.methodDeclarator) return params;

        const declarator = header.children.methodDeclarator[0];
        if (!declarator?.children?.formalParameterList) return params;

        const fpl = declarator.children.formalParameterList[0];
        if (!fpl?.children?.formalParameter) return params;
    ...
    private extractDetailedConstructor(
        node: any,
        modifiers: string[],
        className: string,
        fieldNames: Set<string>
    ): Constructor | null {
        if (!node?.children?.constructorDeclarator) return null;

        const declarator = node.children.constructorDeclarator[0];

        // Constructor name must match class name
        const constructorName = this.getSimpleTypeName(declarator);
        if (!constructorName || constructorName !== className) return null;

        const accessModifier = this.getAccessModifier(modifiers) as Constructor['accessModifier'];
        ...
        let constructorType: Constructor['constructorType'] = 'parameterized';
        ...
        let chainedConstructor: string | undefined;
    ...
    private detectConstructorInjection(
        constructorBody: any,
        parameters: Parameter[],
        fieldNames: Set<string>
    ): InjectionAssignment[] {
        const assignments: InjectionAssignment[] = [];
        if (!constructorBody) return assignments;
    ...
    private findAssignmentPatterns(
        node: any,
        parameters: Parameter[],
        fieldNames: Set<string>,
        assignments: InjectionAssignment[]
    ): void {
        if (!node) return;
    ...
    private findChildByName(node: any, name: string): any | null {
        if (!node) return null;
    ...
    private analyzeConstructorBody(constructorBody: any, parameters: Parameter[]): { calledMethods: MethodCall[]; createdObjects: ObjectCreation[] } {
        if (!constructorBody) {
        ...
        const calledMethods: MethodCall[] = [];
        const createdObjects: ObjectCreation[] = [];
    ...
    private getDetailedParameters(declarator: any, fieldNames: Set<string>): Parameter[] {
        const params: Parameter[] = [];
    ...
    private extractInterfaceMethod(node: any): any | null {
        if (!node?.children?.methodHeader) return null;
    ...
    private getMethodName(header: any): string {
        if (!header?.children?.methodDeclarator) return '';
        const declarator = header.children.methodDeclarator[0];
        if (!declarator?.children?.Identifier) return '';
        return declarator.children.Identifier[0].image;
    }

    private getSimpleTypeName(node: any): string {
        if (!node?.children?.simpleTypeName) return '';
    ...
    private getReturnType(header: any): string {
        if (!header?.children?.result) return 'void';
    ...
    private getMethodParameters(header: any): any[] {
        const params: any[] = [];
    ...
    private getConstructorParameters(declarator: any): any[] {
        const params: any[] = [];
    ...
    private getParameterType(fp: any): string {
    ...
    private getParameterModifiers(fp: any): string[] {
        const modifiers: string[] = [];
        if (!fp?.children) return modifiers;

        if (fp.children.final) modifiers.push('final');
        if (fp.children.annotations) modifiers.push('annotated');
    ...
    private extractAnnotations(node: any): Annotation[] {
        const annotations: Annotation[] = [];

        if (!node?.children?.annotations) return annotations;

        const annotationsNode = node.children.annotations[0];
        if (!annotationsNode?.children?.annotation) return annotations;
    ...
    private extractAnnotationElements(ann: any): { [key: string]: any } {
        const elements: { [key: string]: any } = {};

        if (!ann?.children?.elementValuePair) return elements;
    ...
    private extractElementValue(node: any): any {
        if (!node) return null;
    ...
    private isBuiltInAnnotation(fqName: string): boolean {
        const builtIns = [
            'java.lang.Override',
    ...
    private extractMethodAnnotations(node: any): Annotation[] {
        const annotations: Annotation[] = [];
        if (!node?.children?.methodModifier) return annotations;
    ...
    private extractAnnotation(ann: any): Annotation | null {
        const fqName = this.getAnnotationName(ann);
    ...
    private extractParameterAnnotations(fp: any): Annotation[] {
        const annotations: Annotation[] = [];
    ...
    private extractConstructorAnnotations(node: any): Annotation[] {
        const annotations: Annotation[] = [];
    ...
    private getAnnotationName(ann: any): string {
        if (!ann?.children) return '';
    ...
    private extractJavaDoc(node: any): string | undefined {
    ...
    private extractFieldJavaDoc(vd: any): string | undefined {
        // Similarly, javadoc extraction requires source code with comment positions
        return undefined;
    }

    private getAccessModifier(modifiers: string[]): 'public' | 'private' | 'protected' | 'package-private' {
    ...
    private extractGenericTypeParameters(node: any): GenericTypeParam[] {
        const typeParams: GenericTypeParam[] = [];
    ...
    private getTypeParameterName(param: any): string {
        if (!param?.children?.Identifier) return 'T';
        return param.children.Identifier[0].image;
    }

    private getTypeParameterBound(param: any): string | undefined {
        if (!param?.children?.typeBound) return undefined;
        const bound = param.children.typeBound[0];
        if (!bound?.children?.referenceType) return undefined;
        return this.extractClassName(bound.children.referenceType[0]);
    }

    private determineVariance(param: any): 'invariant' | 'covariant' | 'contravariant' {
    ...
    private getBlockStatements(block: any): any[] {
    ...
    private isStandardLibrary(className: string): boolean {
        return className.startsWith('java.') || className.startsWith('javax.') || className.startsWith('jakarta.');
    }

    private analyzeStatementForCalls(
        statement: any,
        callback: (callType: 'this' | 'super' | 'method', targetClass?: string, methodName?: string) => void
    ): void {
        if (!statement) return;
        ...
        if (statement.name === 'primary') {
            ...
            if (suffix?.children?.methodInvocationSuffix) {
                ...
                if (prefix?.children?.This) { callback('this'); return; }
                if (prefix?.children?.Super) { callback('super'); return; }
    ...
    private collectFqnIdentifiers(fqnNode: any): string[] {
        const ids: string[] = [];
        const collect = (n: any) => {
            if (!n) return;
            if (n.name === 'fqnOrRefTypePartCommon' && n.children?.Identifier) {
    ...
    private inferReceiverTypeByName(receiverName: string): string | undefined {
    ...
    private extractSimpleName(node: any): string {
        if (!node) return '';
        if (node.children?.Identifier) {
    ...
    private extractReceiver(statement: any): any {
    ...
    private inferReceiverType(receiver: any): string | undefined {
    ...
    private extractCreatedClassName(creation: any): string {
        ...
        if (!creation?.children?.createdName) return '';
    ...
    private extractObjectCreations(statement: any): ObjectCreation[] {
        const creations: ObjectCreation[] = [];

        const findCreations = (node: any) => {
            if (!node) return;
    ...
    private extractLocalVariables(statement: any): LocalVariable[] {
    ...
    private getLocalVarNames(statement: any): string[] {
        if (!statement.children?.variableDeclaratorList) return [];

        const vdl = statement.children.variableDeclaratorList[0];
        if (!vdl?.children?.variableDeclarator) return [];

        return vdl.children.variableDeclarator.map((vd: any) => {
            return vd.children?.variableDeclaratorId?.[0]?.children?.Identifier?.[0]?.image || 'unknown';
        });
    }

    private extractEnhancedForVariable(statement: any): LocalVariable | null {
        if (!statement.children?.formalParameter) return null;
    ...
    private extractInitialValue(vd: any): any {
        if (!vd?.children?.variableInitializer) return null;

        const vi = vd.children.variableInitializer[0];
        if (!vi?.children?.expression) return null;

        return this.extractExpressionValue(vi.children.expression[0]);
    }

    private computeJVMDefault(dataType: string): any {
    ...
    private hasGetterMethod(fieldName: string, className: string): boolean {
    ...
    private hasSetterMethod(fieldName: string, className: string): boolean {
        // Similar to getter check
        return false;
    }

    private generateGetterName(fieldName: string, accessModifier: string): string {
    ...
    private generateSetterName(fieldName: string, accessModifier: string): string {
        return `set${fieldName.charAt(0).toUpperCase()}${fieldName.slice(1)}`;
    }

    private postProcessFields(fields: Attribute[], methods: (Method | Constructor)[]): void {
    ...
    private extractThrowsClause(node: any): string[] {
    ...
    private extractExceptionType(et: any): string {
    ...
    private extractChainedConstructorParams(eci: any): string {
    ...
    private extractConstructorBody(node: any): string {
        // Extract constructor body representation
        // Would need source code positions to extract actual code
        return '/* constructor body */';
    }

    private determineMemoryLocation(modifiers: string[]): 'method_area' | 'heap' | 'stack' {
        if (modifiers.includes('static')) return 'method_area';
        return 'heap';
    }

    private getSourceFileName(node: any): string | null {
        // In java-parser, source file info may be available from the parser
        return null;
    }

    private extractPackageInfoFromSource(sourceCode: string): PackageInfo | null {
    ...
    private getModifiers(node: any): string[] {
        const modifiers: string[] = [];
        if (!node?.children) return modifiers;
            ...
            if (node.children[key]) {
                for (const mod of node.children[key]) {
                    if (mod.children) {
                        ...
                        if (mod.children.Strictfp) modifiers.push('strictfp');
                        if (mod.children.Sealed) modifiers.push('sealed');
                        if (mod.children.NonSealed) modifiers.push('non-sealed');
                    }
                }
            }
        }
    ...
    private extractImports(cst: any): ImportInfo[] {
        const imports: ImportInfo[] = [];
        if (!cst) return imports;
    ...
    private parseImportNode(node: any): ImportInfo | null {
    ...
    private getImportName(node: any): string {
        if (!node?.children?.qualifiedName) return '';
        const qn = node.children.qualifiedName[0];
        return this.getIdentifierFromNode(qn) || '';
    }

    private collectExternalDependencies(cls: ClassInfo, imports: ImportInfo[], ownPackage: string | null): string[] {
    ...
    private isExternalType(typeName: string, ownPackage: string | null): boolean {
        if (!typeName || typeName === 'void' || typeName === 'null') return false;
    ...
    private buildMethodCallGraph(cls: ClassInfo): { [methodName: string]: MethodCall[] } {
        const graph: { [methodName: string]: MethodCall[] } = {};
    ...
    private classifyClass(cls: ClassInfo): void {
        const annotationNames = cls.annotations.map(a => a.name);
        const stereotypes: string[] = [...annotationNames];
        let layer: ClassInfo['detectedLayer'] = 'unknown';
    ...
    private extractLocation(nodeOrToken: any): { startLine?: number, startColumn?: number, endLine?: number, endColumn?: number } | undefined {
        if (!nodeOrToken) return undefined;
    ...
    private extractComplexityMetrics(methodBody: any): ComplexityMetrics {
        const metrics: ComplexityMetrics = {
            cyclomaticComplexity: 1,
            maxNestingDepth: 0,
            decisionPoints: []
        };
        if (!methodBody) return metrics;

        let currentDepth = 0;

        const getLine = (node: any): number => {
            const loc = this.extractLocation(node);
            return loc?.startLine ?? 0;
        };

        const getConditionText = (node: any): string => {
            if (!node?.children) return '';
            for (const key of Object.keys(node.children)) {
                if (Array.isArray(node.children[key])) {
                    for (const child of node.children[key]) {
    ...
    private extractConditionText(node: any): string {
        if (!node) return '';
        const tokens: any[] = [];
        const collect = (n: any) => {
            if (!n) return;
        ...
        if (matchingRParenIndex === -1) return '';
