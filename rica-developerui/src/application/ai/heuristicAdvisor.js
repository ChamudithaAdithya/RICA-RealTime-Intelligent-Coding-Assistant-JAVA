...
exports.runHeuristicAdvisor = runHeuristicAdvisor;
 ...
 */
function runHeuristicAdvisor(candidates, filesMap, graph, opts) {
    const { violationConfidence, cleanConfidence } = { ...DEFAULT_OPTS, ...(opts ?? {}) };
    const decisions = [];
    for (const candidate of candidates) {
        if (candidate.featureType === 'ambiguity')
            continue;
        const path = (0, contextBuilder_1.buildCandidatePath)(candidate, filesMap, graph);
        const entry = path[0];
        if (!entry)
            continue;
        const pathHasAuth = path.some(s => s.hasAuthAnnotation);
        decisions.push(pathHasAuth
            ? {
...
//# sourceMappingURL=heuristicAdvisor.js.map