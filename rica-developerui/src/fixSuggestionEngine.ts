...
import { VIOLATION_DOC_BY_CODE } from './violationCatalog';
...
function suggestion(
  v: Violation,
  title: string,
  safety: RemediationSuggestion['safety'],
  steps: string[],
  description?: string,
  ...
  return {
    title,
    safety,
    description: description || VIOLATION_DOC_BY_CODE[v.code || '']?.mitigationHint || v.mitigationHint,
    steps,
  };
}

function annotationSuggestion(v: Violation, annotation: string, title: string, description: string): RemediationSuggestion | undefined {
  const line = fieldLine(v);
  if (!line || !v.filePath) return undefined;
  return {
    title,
    description,
...
const DESIGN_PATTERN_STEPS: Record<string, string[]> = {
...
export class FixSuggestionEngine {
  enrich(violations: Violation[]): Violation[] {
    return violations.map(v => this.enrichOne(v));
  }

  private enrichOne(v: Violation): Violation {
  ...
  private toPreferredQuickFix(suggestions: RemediationSuggestion[]): AiQuickFix | undefined {
  ...
  suggest(v: Violation): RemediationSuggestion[] {
    switch (v.code) {
      case 'RICA-V102':
        return [
      ...
      default:
        if (v.code && DESIGN_PATTERN_STEPS[v.code]) {
          return [suggestion(v, `Refactor ${VIOLATION_DOC_BY_CODE[v.code]?.name || v.code}`, 'manual-design-required', DESIGN_PATTERN_STEPS[v.code])];
        }
