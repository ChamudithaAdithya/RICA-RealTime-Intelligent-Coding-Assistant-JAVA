...
export class JavaParser {
    ...
    private getSuperInterfaces(node: any): string[] {
        if (!node?.children?.superinterfaces) return [];
    ...
    private getExtendsInterfaces(node: any): string[] {
        if (!node?.children?.extendsInterfaces) return [];
    ...
    private getMethodModifiers(node: any): string[] {
        const modifiers: string[] = [];
        if (!node?.children?.methodModifier) return modifiers;
    ...
    private extractParameterAnnotations(fp: any): Annotation[] {
        ...
        // java-parser 2.x: annotation nodes are directly in the formalParameter children
        if (fp?.children?.annotation) {
            for (const ann of fp.children.annotation) {
                const annObj = this.extractAnnotation(ann);
                if (annObj) annotations.push(annObj);
            }
        }
    ...
    private inferReceiverTypeByName(receiverName: string): string | undefined {
        // Simple receiver: first segment is the variable/field name
        const firstSegment = receiverName.split('.')[0];
        if (!firstSegment) return undefined;
        // We can't resolve the type without symbol table here, just return the name as hint
        return undefined;
    }
