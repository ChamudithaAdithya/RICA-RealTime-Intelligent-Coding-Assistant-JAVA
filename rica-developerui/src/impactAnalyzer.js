...
class ImpactAnalyzer {
     ...
     */
    static updateMapsForFile(filePath, _oldAst, newAst, files, maps) {
        ...
        for (const [fp, ast] of Object.entries(files)) {
            ...
            if (depends)
                newDependents.add(fp);
        }
        // 3. Sync dependents sets with the recomputed truth
        const oldDependents = maps.dependents.get(filePath) || new Set();
        ...
        for (const parent of newDependents) {
            if (!oldDependents.has(parent)) {
                if (!maps.dependencies.has(parent))
                    maps.dependencies.set(parent, new Set());
