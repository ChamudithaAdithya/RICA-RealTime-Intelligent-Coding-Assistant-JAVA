...
class PackageBoundaryAnalyzer {
    ...
    analyze(astOutputs, _graph, classAnnotations) {
        ...
        for (const fileAst of astOutputs) {
            ...
            for (const relationship of fileAst.relationships || []) {
                if (!fileAst.classes.some(cls => cls.fullyQualifiedName === relationship.sourceId))
                    continue;
                const targetFqn = this.resolveRelationshipTarget(relationship.targetId, fileAst.packageInfo?.name || '', classesByName);
                if (!targetFqn)
                    continue;
    ...
    resolveRelationshipTarget(targetId, ownPackage, classesByName) {
        const normalized = targetId.replace(/<.*>/g, '').trim();
        if (!normalized)
            return null;
        if (normalized.includes('.'))
            return normalized;
        const samePackage = classesByName.get(normalized)?.filter(fqn => fqn.substring(0, fqn.lastIndexOf('.')) === ownPackage) || [];
