...
function showCST(node: any, depth = 0): void {
        ...
        for (const key of Object.keys(node.children)) {
            ...
            if (Array.isArray(val)) {
                ...
                for (let i = 0; i < Math.min(val.length, 2); i++) {
                    const c = val[i];
                    if (c.children) showCST(c, depth + 2);
                    else if (c.image) console.log(`${indent}    [${i}] ${c.image}`);
                    else console.log(`${indent}    [${i}] (leaf)`);
                }
                if (val.length > 2) console.log(`${indent}    ... (${val.length - 2} more)`);
            } else {
                console.log(`${indent}  ${key}: (scalar)`);
                if (val.children) showCST(val, depth + 1);
            }
        }
    }
}
        ...
        if (imp.children) {
            ...
            if (ptn) {
                ...
                if (ptn.children?.Identifier) {
                    console.log(`  Identifiers: ${ptn.children.Identifier.map((id: any) => id.image).join('.')}`);
                }
            }
        }
    }
...
function findAnnotationContext(node: any): void {
    if (!node) return;
